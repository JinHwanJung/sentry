...
class SubscriptionProcessor(object):
    ...
    def __init__(self, subscription):
        ...
        self.last_update, self.alert_triggers, self.resolve_triggers = self.get_alert_rule_stats()
    ...
    @property
    def alert_rule_stat_keys(self):
        key_base = self.ALERT_RULE_BASE_STAT_KEY % self.alert_rule.id
        return [key_base % stat_key for stat_key in self.ALERT_RULE_STAT_KEYS]

    def get_alert_rule_stats(self):
        """
        ...
        """
        results = self.client.mget(self.alert_rule_stat_keys)
    ...
    def update_alert_rule_stats(self):
        ...
        :return:
        """
        last_update_key, alert_trigger_key, resolve_trigger_key = self.alert_rule_stat_keys
        if self.alert_triggers != self.orig_alert_triggers:
            self.pipeline.set(alert_trigger_key, self.alert_triggers, ex=self.REDIS_TTL)
        if self.resolve_triggers != self.orig_resolve_triggers:
            self.pipeline.set(resolve_trigger_key, self.resolve_triggers, ex=self.REDIS_TTL)

        self.pipeline.set(last_update_key, self.last_update, ex=self.REDIS_TTL)
